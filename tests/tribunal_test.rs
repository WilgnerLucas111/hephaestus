...
async fn test_tribunal_integration() -> Result<()> {
    ...
    let now = SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let mut genome = RepairGenome::new(
        "test_tribunal_hash".to_string(),
        "fn main() { panic!(\"test\"); }".to_string(),
    );
    genome.timestamp = now;

    // Set up dummy genome store for WildMonkey queries
    ...
    let store = hephaestus::memory::genome_store::GenomeStore::open(&db_path)?;
    ...
    let patches = wild_monkey
        .generate_patches(&mut genome, telemetry, &store_arc)
        .await?;
    ...
    assert_eq!(
        ...
        2,
        "Genome should store hypotheses"
    );
    ...
    assert_eq!(
        trial_results.len(),
        2,
        "Should have results for both patches"
    );

    // Step 3: Neutral Judge inspects safeguards
    let _ = neutral_judge.inspect_safeguards(patches.clone()).await?;
    ...
    assert!(
        genome.telemetry_trigger.is_some(),
        "Telemetry trigger should be set"
    );
    assert!(
        genome.narrative_summary.is_some(),
        "Narrative summary should be set"
    );

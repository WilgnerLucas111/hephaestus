use hephaestus::{
    interceptor::interceptor::{
        HephaestusInterceptor, InterceptorConfig, PermissionMode, Skill, SkillResult,
    },
    memory::genome_store::{GenomeStore, RejectionRecord, RepairGenome},
...
async fn test_genome_store_operations() {
    ...
    let now = SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let mut test_genome = RepairGenome::new("test_skill_hash", "fn test() { panic!() }");
    test_genome.telemetry_trigger = Some("panic: index out of bounds".to_string());
    test_genome.monkey_hypotheses = vec!["fn test() { }".to_string()];
    test_genome.rejected_patches = vec![RejectionRecord {
        patch: "fn test() { let x = 1; }".to_string(),
        reason: "does not match original behavior".to_string(),
        timestamp: now,
    }];
    test_genome.final_repaired_code = Some("fn test() { }".to_string());
    test_genome.narrative_summary = Some("The Wild Monkey generated a hypothesis. The Neutral Judge found it safe. The Angry Master applied no penalties. The Narrative Agent recorded the verdict.".to_string());
    test_genome.timestamp = now;

    // Store the genome
    genome_store
        .store_genome(&mut test_genome)
        .expect("Failed to store genome");

    // Retrieve the genome by hash
    let retrieved = genome_store
        .get_genome(&test_genome.hash)
    ...
    genome_store
        .store_genome(&mut updated_genome)
        .expect("Failed to update genome");

    let updated = genome_store
        .get_genome(&test_genome.hash)
    ...
    if let Some(ref r) = updated {
        assert_eq!(
            ...
            Some("fn test() { let x = 42; }".to_string())
        );
    ...
    genome_store
        .delete_genome(&test_genome.hash)
        .expect("Failed to delete genome");

    let deleted = genome_store
        .get_genome(&test_genome.hash)
        .expect("Failed to get genome after deletion");

    assert!(
        deleted.is_none(),
        "Expected genome to be not found after deletion"
    );
...
async fn test_bifucated_agent_skill_execution_does_not_spawn_background_task_on_success() {
    ...
    let interceptor = HephaestusInterceptor::new(event_sender, interceptor_config);
    ...
    let mut bif_agent =
        BifurcatedAgent::new(config, genome_store).expect("Failed to create BifurcatedAgent");
    ...
    let skill_result = bif_agent
        .executeskill_bifurcated(&skill, &interceptor)
        .await;
    ...
    let skill_result2 = bif_agent
        .executeskill_bifurcated(&skill, &interceptor)
        .await;
    assert!(
        matches!(skill_result2, Ok(SkillResult::Success)),
        "Expected Ok(SkillResult::Success) on second execution, got {:?}",
        skill_result2
    );
...
async fn test_bifucated_agent_panicking_skill_triggers_background_repair_attempt() {
    ...
    assert!(
        matches!(skill_result, Ok(SkillResult::Failure(_))),
        "Expected skill execution to fail with error"
    );
    ...
    let skill_result2 = bif_agent
        ...
        .await;
    assert!(
        matches!(skill_result2, Ok(SkillResult::Failure(_))),
        "Expected skill execution to fail with error on second execution"
    );
